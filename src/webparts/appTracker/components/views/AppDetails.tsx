...
import { DocumentType, IApplicationItem, IContractItem, IDocumentItem } from "../data/props";
...
import { ApplicationOverview } from './appDetails/AppOverview';
import { SupportingInfo } from './appDetails/AppSupportingInfo';
import { ContractInfo } from './appDetails/AppContract';
import { Security } from '../services/Security';
import { fetchFileDataUrlFromDocumentItem } from '../export/ExportPdfUtils';
import { exportApplicationPdf } from '../export/ExportPdfWrapper';
...
// Application form style override
...
export const AppDetails: React.FC<AppDetailsProps> = ({ application, context, onBack, onHome, activeTab = "overview", onTabChange }) => {

    const [appState, setAppState] = useState<IApplicationItem>(application);
    ...
    const getDocs = async (appId: number): Promise<boolean> => {
        try {
            const docs = await DataSource.getDocumentsByApplication(appId);
    ...
    const contract: IContractItem | undefined = React.useMemo(() => {
        const cId: number | undefined = appState.contract?.Id;
    ...
    }, [appState.contract, appState.contract?.Id]);

    const relatedContractApps: IApplicationItem[] = React.useMemo(() => {
        const contractLead = (contract?.contractTeam ?? appState.contract?.contractTeam ?? "").trim();

        if (!contractLead) {
            return [];
        }

        return DataSource.Apps
            .filter((app) => app.Id !== appState.Id)
            .filter((app) => {
                const appContractLead = (
                    app.contract?.contractTeam ??
                    DataSource.Contracts.find((c) => c.Id === app.contract?.Id)?.contractTeam ??
                    ""
                ).trim();

                return appContractLead.toLowerCase() === contractLead.toLowerCase();
            })
            .sort((a, b) => a.Title.localeCompare(b.Title));
    }, [appState.Id, appState.contract?.contractTeam, contract?.contractTeam]);

    useEffect(() => {
        getDocs(appState.Id).catch((error) => {
            console.error("Unhandled promise rejection:", error);
        });
    }, [appState.Id]);

    const statusProps = getAtoStatusFill(appState.appStatus);
    ...
    ****************************************************************************/
    ...
    const uploadFile = (fileName: string, file: CustomFile): Promise<IDocumentItem> => {
        return new Promise((resolve, reject) => {
            ...
            const targetFolderUrl = `${ContextInfo.webServerRelativeUrl}/${Strings.Lists.Documents}/${application.Id}`;
                        ...
                        uploadedFile.ListItemAllFields().execute((listItem) => {
                            ...
                            list.Items().getById(newItemId).update({
                                ...
                                () => {
                                    ...
                                    setSpinnerMessage("Updating File Properties");
                                    list.Items().query({
                                        Select: ["File_x0020_Type", "UniqueId", "Id", "FileLeafRef", "application/Id", "ServerRedirectedEmbedUrl"],
                                        Expand: ["application"],
                                        ...
                                        items => {
                                            if (items && items.results && Array.isArray(items.results)) {
                                                ...
                                                setShowSpinner(false);
                                                setDialogProps("Error", "No application documents found or unexpected data structure");
                                                reject(new Error("No application documents found or unexpected data structure"));
                                            }
    ...
    const handleOnAddDocument = async (): Promise<void> => {
        try {
            ...
            if (docFolderStatus !== "ready") {
                ...
                const isFolderPresent = await DocumentService.ensureAppDocumentFolder(appState.Id);
                if (isFolderPresent) {
                    setDocFolderStatus("ready");
                } else {
                    try {
                        await DocumentService.createApplicationFolder(appState.Id);
                    } catch (error) {
    ...
    const isPoc = 
        ...
        contract?.secondaryPoc?.Id === Security.currentUserID || 
        appState.Author?.Id === Security.currentUserID;
    const isPrimaryPoc = appState.primaryPoc?.Id === Security.currentUserID;
    const isCor = contract?.cor?.Id === Security.currentUserID || contract?.acor?.Id === Security.currentUserID;
    const canViewContract = Security.IsAdmin || isPrimaryPoc || isCor;
    const canEdit = Security.IsAdmin || isPoc || isPrimaryPoc;
    ...
    return (
        <div className={styles.applicationTracker}>
                ...
                <Stack className={styles.recordShell}>
                    <Stack horizontal horizontalAlign="space-between" verticalAlign="start" tokens={{ childrenGap: 16 }} className={styles.recordHeader}>
                        <Stack verticalAlign="center" tokens={{ childrenGap: 4 }}>
                            <Stack horizontal wrap verticalAlign="center" tokens={{ childrenGap: 10 }}>
                                <Text variant="xxLarge" styles={{ root: { fontWeight: 600 } }}>{appState.Title}</Text>
                                <Pill
                                    text={appState.appStatus}
                        ...
                        <Stack horizontal tokens={{ childrenGap: 4 }}>
                            ...
                            <DefaultButton
                                text="Export"
                                title="Export Application to a single-page PDF"
                    ...
                    <Stack className={styles.recordContent}>
                        ...
                        {activeTab === "supporting" && (
                            <SupportingInfo appState={appState} />
                        )}

                        {activeTab === "contract" && canViewContract && (
                            <ContractInfo contract={contract} relatedApplications={relatedContractApps} />
                        )}
            ...
            <Dialog
                ...
                onDismiss={() => setShowApplicationForm(false)}
                dialogContentProps={{
                    type: DialogType.largeHeader,
                    title: `Edit Application - ${appState.Title}`,
            ...
            >
                <AppForm
                    ...
                    onSave={async (app) => {
                        try {
                            setSpinnerProps("Editing Application...");
                            const updatedApplication = await AppService.edit(app);
                            setAppState(updatedApplication);
                            setShowApplicationForm(false);
                            setShowSpinner(false);
                        } catch (err) {
                            setShowSpinner(false);
                            console.error(`Error editing Application: ${formatError(err)}`);
                            setDialogProps("Error editing Application", formatError(err));
                        }
                    }}
                    onDelete={async () => {
                        setSpinnerMessage("Deleting Application...");
                        setShowSpinner(true);
                        try {
                            await AppService.delete(appState.Id);
                            //setAppState(undefined);
                            setShowApplicationForm(false);
                            setShowSpinner(false);
                            onBack();
                        } catch (error) {
                            console.error(`Error deleting Application: ${formatError(error)}`);
                            setShowSpinner(false);
                            setDialogProps("Error deleting Application", formatError(error));
                        }
                    }}
