...
const styles = StyleSheet.create({
    ...
    relatedInfoCard: {
        backgroundColor: Strings.PillStyles.Smalt,
        color: "#FFFFFF",
        borderRadius: 12,
        padding: 10,
        marginBottom: 10
...
const joinPeople = (people?: { results: { Title: string }[] }): string =>
...
const ApplicationOnePager: React.FC<IApplicationOnePagerProps> = (props) => {
    const { application, contract, kgsLogoDataUrl, screenshotBinary } = props;

    const description = (application.description ?? "").trim();

    const technicalInfoList: string[] = [
        application.appStatus ? `App Status: ${application.appStatus}` : "",
        application.appLaunchDate ? `App Launch Date: ${formatDate(application.appLaunchDate)}` : "",
        application.connectivity ? `Connectivity: ${application.connectivity}` : "",
        application.licenseReqd ? `License Required? ${application.licenseReqd}` : "",
        application.platform ? `Platform: ${application.platform}` : "",
        Number.isFinite(application.userCount) ? `User Count: ${application.userCount}` : "",
    ].filter(Boolean);

    const stakeholders = joinPeople(application.stakeholders);
        ...
        <Page size="LETTER" orientation="portrait" style={styles.page} wrap={false}>
            ...
            <View style={styles.header}>
                <View style={styles.headerTitleWrap}>
                    ...
                    {/* <Text style={styles.subtitle}>Application One-Page Summary</Text> */}
                </View>
            ...
            <View style={styles.body}>
                <View style={styles.leftCol}>
                    ...
                    <View style={[styles.para, { marginTop: 4 }]}>
                        <View style={styles.labelRow}>
                            <Text style={styles.label}>Primary POC</Text>
                            <Text style={styles.value}>{application.primaryPoc?.Title ?? "—"}</Text>
                        </View>

                        <View style={styles.labelRow}>
                            <Text style={styles.label}>Stakeholders</Text>
                            <Text style={styles.value}>{stakeholders || "—"}</Text>
                        </View>

                        <View style={styles.labelRow}>
                            <Text style={styles.label}>System Owner</Text>
                            <Text style={styles.value}>{application.systemOwner?.Title || "—"}</Text>
                        </View>

                        <View style={styles.labelRow}>
                            <Text style={styles.label}>Contractor Name</Text>
                            <Text style={styles.value}>{application.supportTeam || "—"}</Text>
                        </View>

                        <View style={styles.labelRow}>
                            <Text style={styles.label}>Contract Lead</Text>
                            <Text style={styles.value}>{contract?.contractTeam ?? "—"}</Text>
                        </View>

                        <View style={styles.labelRow}>
                            <Text style={styles.label}>Managing Group</Text>
                            <Text style={styles.value}>{application.managingGroup || "—"}</Text>
                        </View>
                        ...
                        <View style={styles.labelRow}>
                            ...
                            <View style={styles.value}>
                                {application.appUrl ? (
                                    <Link src={application.appUrl} style={styles.link}>
                                        Application URL
                                    </Link>
                                ) : (
                                    <Text>—</Text>
                ...
                <View style={styles.rightCol}>
                    {/* <View style={styles.kpiCard}>
                        ...
                        {renderRichText(application.highlights, { color: "#FFFFFF" })}
                    </View> */}
            ...
            <View style={styles.footer}>
                <Text style={styles.footerLeft}>{shortTitleFooter(application.Title)}</Text>
                <Text style={styles.footerCenter}>UNCLASSIFIED</Text>
                <Text style={styles.footerRight}>{formatDate(application.Modified ?? "")}</Text>
            </View>
        </Page>
    );
};
