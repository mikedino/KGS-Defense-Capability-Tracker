...
export async function exportToExcel(capabilities: ICapabilityItem[]): Promise<true> {
  ...
  worksheet.columns = [
    ...
    { header: "Capability Type - Tier 1", key: "capabilityTypeTier1", width: 44 },
    { header: "Capability Type - Tier 2", key: "capabilityTypeTier2", width: 44 },
  ...
  capabilities.forEach((cap) => {
    const relatedContracts = DataSource.Contracts
      .filter((contract) => ContractService.isLinkedToCapability(contract, cap.Id))
    ...
    const row = worksheet.addRow({
      ...
      notes: cap.notes ?? "",
      capabilityTypeTier1: DataSource.getConfigText("capabilityType", cap.capabilityTypeTier1),
      capabilityTypeTier2: DataSource.getConfigText("capabilityType", cap.capabilityTypeTier2),
